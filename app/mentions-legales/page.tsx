...
const PAGE_SLUG = "mentions-legales";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title =
    page?.seo_title ||
    "Mentions légales | The Wine Watchers";
...
function renderParagraph(
  ...
  if (sectionTitle === "Protection des données personnelles") {
    return (
      <p
        key={key}
  ...
  if (sectionTitle === "Cookies") {
    return (
      <p
        key={key}
        className="mt-4 whitespace-pre-line leading-8 text-[#6d5b50]"
      >
        L&apos;utilisation des cookies et autres traceurs est détaillée dans
        la{" "}
        <Link
