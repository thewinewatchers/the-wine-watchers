...
const legacyPublicSlugs = new Set([
...
function getPublicHref(slug: string) {
  if (legacyPublicSlugs.has(slug)) {
    return `/${slug}`;
  }
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div>
            ...
            <p className="mt-4 max-w-3xl leading-7 text-neutral-600">
              Créez et modifiez les pages institutionnelles et informatives
              du site sans intervenir directement dans le code.
            </p>
          </div>

          <Link
            href="/admin/pages/nouvelle"
            className="rounded-full bg-black px-6 py-3 text-sm font-semibold uppercase tracking-[0.18em] text-white hover:bg-[#8a6a2f]"
          >
            + Créer une page
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="mt-10 space-y-4">
            {pages.map((page) => {
              const publicHref = getPublicHref(page.slug);
