...
function getPriorityTone(stars: number) {
  if (stars >= 5) {
    return {
                  ...
                  <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                    <div>
                      <p className="text-sm uppercase tracking-[0.22em] text-[#8a6a2f]">
                        Centre de Priorités SEO
                      </p>

                      <h2 className="mt-2 font-serif text-3xl text-black md:text-4xl">
                        Mission du jour
                      </h2>

                      <p className="mt-4 max-w-3xl text-sm leading-7 text-neutral-700">
                        Ces requêtes sont classées automatiquement selon leur
                        proximité de la première page, leur volume
                        d’impressions, leur CTR et leur potentiel de clic.
                      </p>
                    </div>

                    <div className="rounded-2xl border border-[#eee2cf] bg-[#fffaf3] px-5 py-4">
                      <p className="text-xs uppercase tracking-[0.16em] text-neutral-500">
                        Missions prioritaires
                      </p>
                      <p className="mt-2 font-serif text-4xl text-black">
                        {data.priorityMissions.length}
                      </p>
                    </div>
                  </div>

                  {data.priorityMissions.length === 0 ? (
                    <div className="mt-8 rounded-2xl border border-green-200 bg-green-50 p-5 text-sm text-green-900">
                      Aucune mission prioritaire détectée sur cette période.
                    </div>
                  ) : (
                    <div className="mt-8 space-y-5">
                      {data.priorityMissions.map((mission, index) => {
                        const tone = getPriorityTone(mission.priorityStars);

                        return (
                          <article
                            key={`${mission.query}-${index}`}
                            className={`rounded-3xl border p-5 md:p-6 ${tone.border} ${tone.background}`}
                          >
                            <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                              <div className="min-w-0">
                                ...
                                <p className={`mt-2 text-lg ${tone.text}`}>
                                  {mission.priorityStarsLabel}
                                </p>
                              </div>

                              <div className="flex flex-wrap items-center gap-3">
                                <span className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-black">
                                  Score : {mission.priorityScore}/120
                                </span>
                            ...
                            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                              ...
                              <div className="rounded-2xl border border-white/80 bg-white p-4">
                                ...
                                <p className="mt-2 font-serif text-2xl text-black">
                                  {formatInteger(mission.impressions)}
                                </p>
                              </div>
                            ...
                            <div className="mt-5 rounded-2xl border border-white/80 bg-white p-5">
                              <p className="text-sm font-semibold text-black">
                                Pourquoi cette mission est prioritaire
                              </p>

                              {mission.priorityReasons.length === 0 ? (
                                <p className="mt-3 text-sm text-neutral-600">
                                  Aucun facteur particulier n’a été détecté.
                                </p>
                              ) : (
                                <ul className="mt-3 space-y-2 text-sm leading-6 text-neutral-700">
                                  {mission.priorityReasons.map(
                                    (reason, reasonIndex) => (
                                      <li
                                        key={`${mission.query}-reason-${reasonIndex}`}
                ...
                <div className="mt-8 grid gap-6 lg:grid-cols-2">
                  <div className="rounded-3xl border border-purple-200 bg-purple-50 p-6 md:p-8">
                    ...
                    <h2 className="mt-3 font-serif text-3xl text-purple-950">
                      Croiser ces missions avec les fiches du catalogue
                    </h2>

                    <p className="mt-4 text-sm leading-7 text-purple-900">
                      La prochaine évolution permettra de relier chaque
                      requête prioritaire à la fiche vin correspondante et
                      d’ouvrir directement sa page de modification.
                    </p>
                  </div>
                  ...
                  <div className="rounded-3xl border border-[#e6dcc8] bg-white p-6 md:p-8">
                    ...
                    <p className="mt-4 text-sm leading-7 text-neutral-700">
                      L’Assistant SEO du catalogue détaille déjà les contenus
                      manquants, les métadonnées trop courtes et les
                      recommandations fiche par fiche.
                    </p>
