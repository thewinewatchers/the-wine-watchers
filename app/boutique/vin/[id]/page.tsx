...
function categoryToSlug(value?: string | null) {
  ...
  if (normalized.includes("champagne")) {
    return "champagne";
  }
  ...
  if (normalized.includes("italie")) {
    return "italie";
  }

  if (normalized.includes("bourgogne")) {
    return "bourgogne";
  }

  if (normalized.includes("bordeaux")) {
    return "bordeaux";
  }

  if (normalized.includes("rhone")) {
    return "rhone";
  }

  if (normalized.includes("espagne")) {
    return "espagne";
  }

  if (normalized.includes("primeur")) {
    return "primeurs-2025";
  }
...
export default async function WinePage({ params }: Props) {
  ...
  if (normalizedId !== id) {
    ...
    if (normalizedWine) {
      permanentRedirect(
        `/boutique/vin/${normalizedWine.slug || normalizedWine.id}`
      );
    }
  }
  ...
  if (id !== slug) {
    permanentRedirect(`/boutique/vin/${slug}`);
  }
